
<?php


// connexion a la base du donnée 
 include 'config.inc.php';
  
 //initiation des variables 
$discussions = array();
$login=$_POST['login'];
$req0 = $conn->prepare('SELECT  id  AS n FROM user WHERE  login=? ');
		$req0->execute(array($login));
				$donnees0 = $req0->fetch();
				$id=$donnees0['n'];


// recherche des etablissements selon le type 
 // creation du requette 
 $req1 = $conn->prepare('SELECT * FROM discussion WHERE  id_2=? OR id_1=? ORDER BY id DESC');

 
 // execution de requette 
 $req1->execute(array($id,$id));

 while ($donnees1 = $req1->fetch(PDO::FETCH_ASSOC)){
	$temp1['Id']=$donnees1['id'];
	//$temp1['id_user']=$donnees1['id_user'];
	$temp1['text']=$donnees1['last_msg'];
	$temp1['time']=$donnees1['time'];
	
	if($donnees1['id_1']!=$id)
	{$req = $conn->prepare('SELECT  nom  AS nom FROM user WHERE  id=? ');
		$req->execute(array($donnees1['id_1']));
		$donnees = $req->fetch();
		$req = $conn->prepare('SELECT  prenom  AS prenom FROM user WHERE  id=? ');
		$req->execute(array($donnees1['id_1']));
				$donnees2 = $req->fetch();

		$req = $conn->prepare('SELECT  image  AS image FROM user WHERE  id=? ');
		$req->execute(array($donnees1['id_1']));
				$donnees3 = $req->fetch();

		$temp1['user1']=$donnees['nom']." ".$donnees2['prenom'];
		
	$temp1['image']=$donnees3['image'];
		$temp1['user2']=$donnees1['id_2'];}
		else{$req = $conn->prepare('SELECT  nom  AS nom FROM user WHERE  id=? ');
		$req->execute(array($donnees1['id_2']));
		$donnees = $req->fetch();
		$req = $conn->prepare('SELECT  prenom  AS prenom FROM user WHERE  id=? ');
		$req->execute(array($donnees1['id_2']));
				$donnees2 = $req->fetch();

		$req = $conn->prepare('SELECT  image  AS image FROM user WHERE  id=? ');
		$req->execute(array($donnees1['id_2']));
				$donnees3 = $req->fetch();

		$temp1['user1']=$donnees['nom']." ".$donnees2['prenom'];
		
	$temp1['image']=$donnees3['image'];
		$temp1['user2']=$donnees1['id_1'];}


	
	array_push($discussions,$temp1);
}

echo json_encode(array("discussions"=>$discussions),JSON_UNESCAPED_UNICODE);
	

?>